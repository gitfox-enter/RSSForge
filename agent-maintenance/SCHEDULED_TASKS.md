...
### 2026-07-19（周日 · 第 2 次三审）

**A. `--deep` 深度维护结果**（commit `51f8aef`，已推送）
- 补全 `feeds_meta.json` 缺失 `url` 字段：**11 个**（与 sites.yaml 的站点 `url` 对齐）。
- 硬死站迁移：**0 个**。原因：11 个零内容活跃站 HTTP 探针全部非硬死（软故障/可用，按设计「软故障只报告不迁移」）。
- 生成周报：`agent-maintenance/weekly-report_2026-07-19.md` + 诊断日志 `logs/health_2026-07-19_22-02.json`。
- 健康基线：活跃 **22** / 死站 **15** / 正常产出 **11** / 零内容 **11** / 健康率 **50.0%**（=50% 告警线临界）。

**B. 本周新代码修复**（commit `c92221f`，已推送，经单元验证）
- **#122** `common.py` `is_junk()`：精确匹配 `clean == junk_word` → 子串匹配 `junk_word in clean`。上周留作行为决策项，本周确认改子串匹配并修复（JUNK_PATTERNS 本就是 UI 噪音词）；6 条单元用例全过。
...
**D. 重要发现**
- 代码内大量 `fix #N` 注释表明活跃开发已修掉一批审计 issue；但 **#122 再次印证「注释/摘要声称已修复」不可尽信**——本周仍以代码逐行核对为准，并顺手把 #122 真修掉（上周的陷阱本周闭环）。
- `#132` 每周维护机制「脚本已实现、可手动/按需运行，但 `.github/workflows/` 尚未接入 `weekly-maintenance.yml` 定时调度」——属部分完成，未假称完全覆盖，保留 open。

**E. 下周计划**
1. 零内容根因治理：11 个零内容活跃站（探针均非硬死，属 crawl 产出/parser 问题）做 parser/SSL 专项排查，拉升 50% 临界健康率。
2. 安全/并发高优专项：`#79`(URL 明文 token) `#54`(开放重定向缺白名单) `#58`(--theirs 丢数据) `#81`/`#128`(并发覆盖) `#83`(push 失败仍 exit 0) `#56`(未提交 items) `#78`(测试被吞)。
3. 接入 `weekly-maintenance.yml` 定时调度，使 #132 完全闭环。
4. 复查 `dead_sites` 中 CONN_TIMEOUT 站点是否恢复，可迁回 active。
